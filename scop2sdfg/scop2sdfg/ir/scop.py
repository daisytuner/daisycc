...
from scop2sdfg.ir.ast import ASTBuilder

from scop2sdfg.ir.computation.computation import Computation
from scop2sdfg.ir.computation.phi import Phi
from scop2sdfg.ir.computation.access import Access

from scop2sdfg.ir.symbols.parameter import Parameter
from scop2sdfg.ir.symbols.memref import Memref
from scop2sdfg.ir.symbols.loop import Loop
from scop2sdfg.ir.symbols.constant import Constant

from scop2sdfg.ir.analysis import (
    value_propagation,
    ...
    def from_json(source: str, desc: Dict) -> Scop:
        ...
        for instruction in desc["instructions"].replace("\\n", "\n").splitlines():
            ...
            if "br" in instruction:
                ...
                if len(tokens) == 3:
                    cond, iftrue, iffalse = tokens
                    cond = cond.split()[-1]
                    iftrue = iftrue.split()[-1]
                    iffalse = iffalse.split()[-1]
                    branch_insts.append((cond, iftrue, iffalse))
                continue
        ...
        for instruction in desc["instructions"].replace("\\n", "\n").splitlines():
            ...
            if "phi " in instruction:
                computation = Phi.from_string(instruction, branch_insts)
                if computation is None:
                    continue
